...
May_inc <- read.csv("Step1_N_incMay_20260516.csv") [, -c(32)]
June_inc <- read.csv("Step1_N_incJune_20260516.csv")
July_inc <- read.csv("Step1_N_incJuly_20260516.csv")
...
merged_data <- bind_rows(May_inc, June_inc, July_inc) %>%
  select(-any_of("X"))
summary(merged_data)
...
#### isolate and check spikes
# isolate only spike incubations
spike_df <- merged_data %>%
  filter(Type == "spike")

# quick check
table(spike_df$Analyte)
table(spike_df$Inc_month)
...
ggplot(spike_df,
  ...
  geom_line(
    aes(group = interaction(Location, Site, Analyte, Inc_month, Composite_num)),
    alpha = 0.4
  ) +
  facet_grid(Analyte ~ Inc_month, scales = "free_y") +
  theme_minimal(base_size = 14) +
  labs(
    x = expression("Spike concentration ("*mu*"g/L)"),
    y = "Measured concentration (mg N/L)",
    color = "Location"
  )

spike_df %>%
  filter(Inc_month == "June") %>%
  arrange(desc(Conc_mgNL)) %>%
  dplyr::select(
    Site, Location, Depth, Vial_num,
    Analyte, Spike_µg_L,
    Conc_mgNL, Notes
  )
...
ammonium_save <- ammonium_df %>%
  dplyr::select(-ammon_cal, -pKa, -f, -NH3_mgNL)
# write.csv(ammonium_save, file = "Step2_NH4corrected_Ninc_data_20260515.csv", row.names = FALSE)
